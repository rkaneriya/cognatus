...
export default function useSharedTreeAPI(fetchTrees) {
  ...
  const fetchSharedTrees = useCallback(async () => {
    ...
    const {
      data: sharedTrees,
      count,
      error: sharedTreeError,
    } = await supabase
      .from(SHARED_TREE_TABLE)
      .select("*", { count: "exact" })
      .eq(SHARED_TREE_TABLE_COLS.SHAREE_EMAIL, user?.email)
      .order(SHARED_TREE_TABLE_COLS.CREATED_AT, { ascending: false })
      .range(start, end);
    ...
    const sharedTreesByUuid = sharedTrees.reduce(
      (acc, s) => ({
        ...acc,
        [s.tree_uuid]: s,
      }),
    ...
    const { data: shareeExtData, error: shareeExtError } = await supabase
      ...
      .select("*")
      .eq(SHAREE_TREE_EXT_TABLE_COLS.SHAREE_EMAIL, user?.email)
      .in(
        SHAREE_TREE_EXT_TABLE_COLS.SHARED_TREE_ROW_UUID,
        sharedTrees.map((t) => t.uuid)
      );
    ...
    const shareeTreeExtBySharedTreeRowUuid = shareeExtData.reduce(
      (acc, s) => ({
        ...acc,
    ...
    const { data: trees, error: treeError } = await supabase
      .from(TREE_TABLE)
      .select("*")
      .in(TREE_TABLE_COLS.UUID, Object.keys(sharedTreesByUuid));
    ...
    if (treeError) {
      ...
      const keyedData = trees.map((tree) => ({
        ...
        is_email_subscribed:
          shareeTreeExtBySharedTreeRowUuid[sharedTreesByUuid[tree.uuid].uuid]
            ?.is_email_subscribed,
        sharee_tree_ext_row_uuid:
          shareeTreeExtBySharedTreeRowUuid[sharedTreesByUuid[tree.uuid].uuid]
            ?.uuid,
      }));
      setData(keyedData);
  ...
  async function createSharedTree(treeUuid, shareeEmail, isEditable) {
    ...
    const payload = {
      [SHARED_TREE_TABLE_COLS.TREE_UUID]: treeUuid,
      [SHARED_TREE_TABLE_COLS.SHARER_EMAIL]: user?.email,
      [SHARED_TREE_TABLE_COLS.SHAREE_EMAIL]: shareeEmail,
      [SHARED_TREE_TABLE_COLS.IS_EDITABLE]: Boolean(isEditable),
    };

    const { error } = await supabase.from(SHARED_TREE_TABLE).insert([payload]);
  ...
  async function deleteSharedTree(uuid) {
    ...
    const { error } = await supabase
      .from(SHARED_TREE_TABLE)
      .delete()
      .eq(SHARED_TREE_TABLE_COLS.UUID, uuid);
  ...
  async function upsertShareeTreeExt(record) {
    ...
    const EDITABLE_FIELDS = [
      "uuid",
      "sharee_email",
      "shared_tree_row_uuid",
      "tree_uuid",
      "is_email_subscribed",
    ];
    const editableFieldsSet = new Set(EDITABLE_FIELDS);

    const payload = Object.keys(record).reduce(
      (acc, field) => {
