...
export default function QueryRelationTab({ onSelectTargetInRelation }) {
  ...
  const relativeOptions = members.filter((m) => m.uuid !== selectedMemberUuid);
  ...
  if (pathPeople.length > 0) {
    ...
    relationStr = (
      <>
        <Typography.Link onClick={() => onSelectTargetInRelation(target)}>
          {target.first_name}
        </Typography.Link>
  ...
  return (
    <>
      ...
      {pathNodes.length > 0 && <Alert message={relationStr} type="error" />}
      {pathNodes.length > 0 && (
        <div className="italic text-xs text-gray-400 mt-2">
          ^ Incorrect relation? Let me know{" "}
          <Typography.Link
            className="text-xs"
