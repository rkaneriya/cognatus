...
const DEMO_ROUTES = [ROUTES.DEMO_ROOSEVELTS, ROUTES.DEMO_BRITISH_ROYALS];

const { ADD_FIRST_MEMBER, EDIT_MEMBER } = MEMBER_RELATION_ACTIONS;
...
function Wrapper({ children }) {
  return <div className="h-screen bg-neutral-100">{children}</div>;
}
...
export default function Tree() {
  ...
  const [initialMemberEditorValues, setInitialMemberEditorValues] = useState(
    DEFAULT_MEMBER_FORM_VALUES,
  );
  const [memberDrawerConfig, setMemberDrawerConfig] =
    useState(ADD_FIRST_MEMBER);

  const [isRelationDrawerOpen, setIsRelationDrawerOpen] = useState(false);
  const [initialRelationEditorValues, setInitialRelationEditorValues] =
    useState({});
  ...
  function handleAddNewMemberAndRelation(config, initialDrawerValues) {
    ...
    setInitialMemberEditorValues(
      initialDrawerValues ? initialDrawerValues : DEFAULT_MEMBER_FORM_VALUES,
    );
  ...
  function handleEditMember() {
    ...
    const initialValues = {
      ...
      birth_date: selectedMember.birth_date
        ? moment(selectedMember.birth_date)
        : undefined, // birth_date should always be defined
      death_date: selectedMember.death_date
        ? moment(selectedMember.death_date)
        : undefined,
    };
  ...
  if (!loading && isTreeEditable === null) {
    return (
      <Result
        ...
        extra={
          <Button type="primary" onClick={() => router.push(ROUTES.ADMIN)}>
            Back to trees
          </Button>
        }
      />
